...
function loadData() {
    try {
        const stored = localStorage.getItem("gamingData");
...
function updateStats(count) {
    ...
    if (elements["en attente"]) elements["en attente"].textContent = count["En attente"];
...
function exportData() {
    ...
    link.href = url;
    link.download = `tournoi-gaming-${new Date().toISOString().split('T')[0]}.json`;
    link.click();
